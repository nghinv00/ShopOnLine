...
function onKeyDown(e) {
	...
	if((key==120)||(key==123)||(key==119)||(key==118)) {
        if (key == 120) {
           var on_off = 1; setMethod(((method == 4) ? 0 : ++method))
        }
        else if (key == 118) {
            setDauCu(((dauCu == 1) ? 0 : 1))
        }
        else if (key == 119) {
            dockspell = (dockspell == 0) ? 1 : 0; setSpell(dockspell)
        }
		else if(key==123) {
...
//if (useCookie == 1) {
//    setCookie = doSetCookie; getCookie = doGetCookie
//}
//else {
//    setCookie = noCookie; getCookie = noCookie
//}
function noCookie() {
}
